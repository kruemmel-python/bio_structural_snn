...
# ------------------------------------------------------------
# Hippocampus-Stack (komplett, biologisch inspiriert):
...
# - Strukturelles Wachstum (EXC->EXC) + Pruning + Overlap-Block
# - Synaptische Skalierung (Homeostase)
# - Theta/Gamma-Oszillator mit phasenabhängiger LTP/LTD-Asymmetrie
# - Kontext-Gating per Ctx->(CA3,CA1) MATRIZEN + LANGSAMES LERNEN (Hebb + L1)
# - CA1-Comparator-Subpopulation (±(DG,CA3)) + Comparator-Feedback
# - Comparator-Feedback moduliert Plastizität (lr) & Replay-Temperatur
# - Schlaf-Replay (Salienz × Rezenz) mit temperaturgewichteter Auswahl
# ------------------------------------------------------------
...
class Hyper:
    ...
    ctx_drive: float = 1.2
    ctx_decay: float = 0.92           # Relaxation neuron-spezifischer ctx_gate -> 1.0
    ctx_gate_min: float = 0.5
    ctx_gate_max: float = 1.8
    ctx_gain_ca3: float = 1.0          # Verstärkung der Matrixprojektion (CA3)
    ctx_gain_ca1: float = 1.0          # Verstärkung der Matrixprojektion (CA1)
    ctx_init_w: Tuple[float, float] = (0.2, 1.0)  # Startgewichte Ctx->Gate
    ctx_lr: float = 0.002              # Lernrate der Gating-Matrizen
    ctx_w_decay: float = 0.0005        # langsamer Zerfall der Matrixgewichte
    ctx_w_min: float = 0.05
    ctx_w_max: float = 2.0
    ctx_l1_target_ca3: float = 30.0    # L1-Norm-Target je Postsynapse (CA3)
    ctx_l1_target_ca1: float = 25.0    # L1-Norm-Target je Postsynapse (CA1)

    # Theta/Gamma + LTP/LTD-Asymmetrie
    theta_period: int = 12
    gamma_period: int = 3
    theta_potent_window: Tuple[float, float] = (0.2, 0.6)  # Phase in [0..1] -> LTP-Boost
    gamma_gate_strength: float = 0.6
    ltp_boost: float = 1.2      # innerhalb Potenzfenster: LTP x1.2, LTD x0.8
    ltd_boost: float = 1.2      # außerhalb: LTD x1.2, LTP x0.8
...
class Oscillator:
    ...
    def gates(self, h: Hyper) -> Tuple[float, float, float]:
        """
        Liefert (stdp_gate, ltp_bias, ltd_bias)
        - stdp_gate:  Gesamtwirksamkeit (Theta+Gamma)
        - ltp_bias:   Faktor für positive Eligibility (LTP)
        - ltd_bias:   Faktor für negative Eligibility (LTD)
        """
        th = self.theta_phase()
        gm = self.gamma_phase()
        ...
        in_potent = (t0 <= th <= t1)
        ltp_bias = h.ltp_boost if in_potent else 1.0 / h.ltp_boost  # <1 außerhalb
        ltd_bias = 1.0 / h.ltd_boost if in_potent else h.ltd_boost  # >1 außerhalb
...
# ==============================
# 4) Layer (EXC/INH, Wachstum, Skalierung, Kontexte, Oszillator)
# ==============================
...
class Layer:
    def __init__(self, n_exc: int, n_inh: int, h: Hyper, osc: Oscillator, seed: Optional[int] = None):
        ...
        # globaler Plastizitäts-Gate (Comparator-Feedback)
    ...
    def _plasticity_update(self, fired_exc: List[int], fired_inh: List[int]) -> None:
        ...
        for idx, s in enumerate(self.syn):
            ...
            if pre_fired:
                s.pre_trace += 1.0
                s.elig -= 0.5 * s.post_trace  # LTD-Anteil (Post vor Pre)
            if post_fired:
                s.post_trace += 1.0
                s.elig += 1.0 * s.pre_trace   # LTP-Anteil (Pre vor Post)
            ...
            # LTP/LTD-Asymmetrie: positiv/negativ unterschiedliche Verstärkung
...
class ContextLayer:
    """Einfache EXC-Population; liefert binäres Spike-Muster (letzter Schritt)."""
...
# ==============================
# 6) HippocampalSystem (mit Ctx-Lernen & Comparator-Feedback)
# ==============================
...
class HippocampalSystem:
    def __init__(self, h: Optional[Hyper] = None, seed: Optional[int] = 7):
        ...
        # Kontext-Gating-MATRIZEN (pro Postsynapse j: Gewichtsvektor über Ctx-Zellen)
    ...
    # --- Kontext-Gating per Matrix berechnen ---
    ...
    # --- Lernen der Kontext-Matrizen (langsam, lokal) ---
    def _learn_context_matrices(self) -> None:
        """
        Hebb-ähnlich: für postsynaptische EXC, die gerade feuern,
        w_j <- w_j + eta * (a - 0.5) - decay
        + L1-Normierung auf Zielsumme
        """
        a = self.Ctx.last_fired_mask  # 0/1
        eta = self.h.ctx_lr
        ...
        for j, n in enumerate(self.CA3.exc):
            if not n.fired:
                # auch ohne Spike leicht ausdünnen (Vergessen)
    ...
    def step(self) -> None:
        ...
        # Kontext: updaten, daraus Gates berechnen
        self.Ctx.step()
        ...
        # Comparator-Feedback anwenden
        self._apply_comparator_feedback(diff_sum_pos, diff_sum_neg)

        # >>> NEU: Lernen der Kontext-Matrizen (langsam, lokal, nach rekurrenter Dynamik)
...
def demo() -> None:
    h = Hyper()
    sys = HippocampalSystem(h, seed=29)
